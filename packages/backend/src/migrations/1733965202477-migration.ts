...
export class Migration1733965202477 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "note" RENAME COLUMN "replyingToIdId" TO "replyIds"`
		);
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(`ALTER TABLE "note" DROP COLUMN "replyIds"`);
		await queryRunner.query(
			`ALTER TABLE "note" ADD "replyIds" character varying`
		);
		await queryRunner.query(
			`ALTER TABLE "note" RENAME COLUMN "replyIds" TO "replyingToIdId"`
		);
		await queryRunner.query(
