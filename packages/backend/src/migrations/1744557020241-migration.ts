...
export class Migration1744557020241 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "poll" ADD "keys" character varying array NOT NULL`
		);
		await queryRunner.query(
			`ALTER TABLE "poll" ADD "values" character varying array NOT NULL`
		);
	}
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "poll" ADD "value" character varying array NOT NULL`
		);
		await queryRunner.query(
			`ALTER TABLE "poll" ADD "key" character varying array NOT NULL`
		);
		await queryRunner.query(
