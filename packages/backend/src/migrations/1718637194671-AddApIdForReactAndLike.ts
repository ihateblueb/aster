...
export class AddApIdForReactAndLike1718637194671 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(
			`ALTER TABLE "notes_react" ADD "ap_id" character varying NOT NULL`
		);
		await queryRunner.query(
			`ALTER TABLE "notes_like" ADD "ap_id" character varying NOT NULL`
		);
	}
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "notes_react" DROP COLUMN "ap_id"`
		);
	}
}
