...
module.exports = class AddMoreToTheInstanceEntity1714572825412 {
	...
	async up(queryRunner) {
		await queryRunner.query(
			`ALTER TABLE "instances" ADD "user_count" integer`
		);
		await queryRunner.query(
			`ALTER TABLE "instances" ADD "note_count" integer`
		);
		await queryRunner.query(
			`ALTER TABLE "instances" ADD "responding" boolean DEFAULT false`
		);
	...
	async down(queryRunner) {
		await queryRunner.query(
			`ALTER TABLE "instances" DROP COLUMN "favicon"`
		);
		...
		await queryRunner.query(
			`ALTER TABLE "instances" DROP COLUMN "responding"`
		);
		await queryRunner.query(
			`ALTER TABLE "instances" DROP COLUMN "note_count"`
		);
		await queryRunner.query(
			`ALTER TABLE "instances" DROP COLUMN "user_count"`
		);
	}
};
